...
The tradeoffs are lifecycle and discipline. A notifier must be created in `initState()` (or owned by a long-lived object) and disposed in `dispose()`, or it leaks listeners. Mutating a field inside the value, such as adding to a list, doesn't notify anyone, which is the same "silent mutator" trap as Bug #2. You must assign a new value. Testability favors `ValueNotifier`: a plain unit test can set `.value` and assert on listeners with no widget tree, while `setState()` logic can only be exercised through `testWidgets` and `tester.tap()`, as this project's tests do. For a single-screen activity like this one, `setState()` is clearer and easier to explain. `ValueNotifier` pays off when one value drives a small part of a large tree, or when several unrelated widgets need to watch the same value. It's also the building block that Provider and Riverpod are based on.
